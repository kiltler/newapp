...
- **Telegram-бот-пульт** (двусторонний): команды `/status`, `/devices`, `/round`
  (коллаж всех камер), `/cam <имя>` + кнопки «перезагрузить/синхр. время» под
  алертами устройства. Long-polling — публичный URL не нужен.
- **Watchdog**: внешний «пульс» (healthchecks.io) — если сам монитор/опрос
  упадёт или зависнет, внешний сервис уведомит вас. «Сторож для сторожа».
...
- **Снимок камеры в Telegram-алерте**: при проблеме с картинкой в чат приходит
  сам проблемный кадр.
- **Детект изменения конфигурации**: камеру добавили/убрали на NVR → событие.
- **TV-режим** (`/tv`): полноэкранный дашборд для дежурки с авто-обновлением и
  звуком на критические проблемы.
- **История** (`/history`): инциденты по дням, топ проблемных камер, лента событий,
  экспорт в CSV.
- **План объекта** (`/plan`): загрузка плана-картинки и расстановка камер точками
  с цветом по статусу.
- **Журнал обслуживания**: заметки/история работ по устройству и каналам.
- **Автобусы** (`/buses`): ручной офлайн-учёт дисковой ротации автобусных DVR —
  автобусы, диски (installed/на просмотре/резерв/неисправен), атомарная «Замена
  диска», журнал замен с CSV. Янтарный режим, без сети/пингов.
- **KPI «потеряно часов записи»** за период по каждой камере (из покрытия архива).
- **Heatmap аптайма** (камера × день) — видно хронически проблемные камеры.
- **Аудит действий** в панели (кто/когда жал перезагрузку, глушил каналы и т.п.).
- **Температура и загрузка NVR** (`/ISAPI/System/status`) + алерт на перегрев.
- **Контроль прошивок** (`/firmware`) с подсветкой «разнобоя» по моделям.
- **Калькулятор хранилища** (`/calc`).
- **Режим монтажника** (`/m/<id>`) + **QR-наклейки** (`/labels`) для работы с телефона.
...
Нативная оболочка на Capacitor (`mobile/`) ставится как обычное приложение
(`.apk`/`.ipa`) и подключается к серверу **внутри локальной сети предприятия из
любого места** через приватную сеть Tailscale (WireGuard) — без белого IP и
проброса портов. Интерфейс не переписан: оболочка открывает уже существующую
веб-панель в webview. Сборка и настройка — в [`mobile/README.md`](mobile/README.md).
